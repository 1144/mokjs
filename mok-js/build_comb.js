	...
	function parseVersion(file, charset){
		...
		if(FS.existsSync(file) && FS.statSync(file).isFile()){
			...
			for(; i < len; i++){
				...
				if(ver.length > 3){
					//文件路径为key，值为数组：[编码后的文件名, 2位版本号，8位文件MD5值，文件名]
...
exports.build = function(argv, prj_conf, response){
	...
	function calcMainfileDL(){
		...
		var main_files = FS.readdirSync(prj_path+'main').sort(),
		...
		for(; i < main_len; i++){
			...
			main_file_dl.push('"'+main_file.slice(0,-3)+'":"'+depended_list.join(',')+'"');
	...
	function compressFiles(){
		...
		if(prj_conf.output_uncompressed){
			for(i = 0; i < len; i++){
				file = all_depended[i];
				fd = FS.openSync(path_uncompress + versions[file][0] + '.js', 'w', '0666');
	...
	function updateBoot(){
		...
		fc = fc.replace('//<VERSIONS/>', 'vers="1:'+versions['mok'][1]+version_fc+'".split(",");'
